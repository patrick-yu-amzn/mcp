...
class K8sHandler:
    ...
    async def apply_yaml(
        ...
        namespace: str = Field(
            ..., description='Namespace to apply resources to (required for namespaced resources)'
        ),
        force: bool = Field(
            True,
            description='Whether to update resources if they already exist (like kubectl apply)',
        ),
    ) -> ApplyYamlResponse:
    ...
    async def manage_k8s_resource(
        ...
        operation: str = Field(
            ...,
            description='Operation to perform (create, replace, patch, delete, read). Use list_k8s_resources for listing resources.',
        ),
        cluster_name: str = Field(..., description='Name of the EKS cluster'),
        ...
        name: Optional[str] = Field(None, description='Name of the Kubernetes resource'),
        namespace: Optional[str] = Field(None, description='Namespace of the Kubernetes resource'),
        body: Optional[Dict[str, Any]] = Field(None, description='Resource definition'),
    ) -> KubernetesResourceResponse:
    ...
    async def list_k8s_resources(
        ...
        namespace: Optional[str] = Field(
            None, description='Namespace of the Kubernetes resources'
        ),
        label_selector: Optional[str] = Field(
            None, description='Label selector to filter resources'
        ),
        field_selector: Optional[str] = Field(
            None, description='Field selector to filter resources'
        ),
    ) -> KubernetesResourceListResponse:
    ...
    async def generate_app_manifest(
        ...
        ctx: Context,
        app_name: str = Field(
            ..., description='Name of the application (used for deployment and service names)'
        ),
        image_uri: str = Field(
            ...,
        ...
        cpu: str = Field(
            '100m', description='CPU request for each container (e.g., "100m" for 0.1 CPU cores)'
        ),
        memory: str = Field(
            '128Mi', description='Memory request for each container (e.g., "128Mi" for 128 MiB)'
        ),
        namespace: str = Field('default', description='Kubernetes namespace to deploy to'),
        load_balancer_scheme: str = Field(
            'internal', description='AWS load balancer scheme (internal or internet-facing)'
        ),
        output_dir: str = Field('output', description='Directory to save the manifest file'),
    ) -> GenerateAppManifestResponse:
        """Generate Kubernetes manifest for a deployment and service.

        This tool generates Kubernetes manifest for a deployment and service for the specified image, with:
        - A load balancer service
        - The service's targetPort matching the container's port
        - Configurable resource requests
        - Configurable number of replicas for high availability
        - Saves the manifest to the specified output directory

        Returns:
            CreateAppManifestResponse: The complete Kubernetes manifest content and output file path
        """
        try:
            log_with_request_id(
                ctx,
    ...
    async def get_pod_logs(
        ...
        container_name: Optional[str] = Field(
            None, description='Container name (optional, if pod contains more than one container)'
        ),
        since_seconds: Optional[int] = Field(
            None, description='Only return logs newer than this many seconds (optional)'
        ),
        tail_lines: int = Field(
            100, description='Number of lines to return from the end of the logs (defaults to 100)'
        ),
        limit_bytes: int = Field(
            10240, description='Maximum number of bytes to return (defaults to 10KB)'
        ),
    ) -> PodLogsResponse:
    ...
    async def get_k8s_events(
        ...
        cluster_name: str = Field(..., description='Name of the EKS cluster'),
        kind: str = Field(
            ..., description='Kind of the involved object (e.g., "Pod", "Deployment")'
        ),
        name: str = Field(..., description='Name of the involved object'),
        namespace: Optional[str] = Field(
            None,
            description='Namespace of the involved object (optional for non-namespaced resources)',
        ),
    ) -> EventsResponse:
        ...
        - First and last timestamps of the events
    ...
    async def list_api_versions(
        ...
        managing Kubernetes resources. This includes both core APIs like 'v1' and
        API groups like 'apps/v1', 'networking.k8s.io/v1', etc.
